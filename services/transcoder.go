...
import (
	...
	"github.com/Nks/go-gcs-ffmpeg/ffmpeg"
...
func (t Transcoder) GetCommand() []string {
	media := t.mediafile
	return append([]string{"-y"}, media.ToString()...)
}
...
func (t *Transcoder) Initialize(inputPath string, parameters []string) error {
	...
	MediaFile.AddInputFile(inputPath)
	for _, parameter := range parameters {
		MediaFile.AddCommand(parameter, false)
	}
...
func (t *Transcoder) Run(progress bool) <-chan error {
	...
	if progress {
		...
		if err != nil {
			...
			fmt.Println("Progress available: ")
	...
	go func(err error, out *bytes.Buffer) {
		if err != nil {
			done <- fmt.Errorf("failed start FFMPEG (%s) with %s, message %s (%s)", command, err, out.String())
			close(done)
			return
		}
		...
		err = proc.Wait()
		if err != nil {
			err = fmt.Errorf("failed finish FFMPEG (%s) with %s message %s (%s)", command, err, out.String())
		}
		done <- err
		close(done)
	}(err, out)
...
func (t Transcoder) Output() <-chan models.Progress {
	...
	go func() {
		defer close(out)
		if t.stdErrPipe == nil {
			fmt.Println("Progress not available")
